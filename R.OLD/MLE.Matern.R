...
MLE.Matern<- function(x,y,smoothness, theta.grid=NULL,
                       ngrid=20, verbose=FALSE,niter=25, tol=1e-5,
...
# (i.e. convex)
  if( is.null(theta.grid)){
    theta.range<- quantile(  local.distance.function( x,x), c( .03,.97))
...
# one final call to Krig with the theta.MLE value to recover MLEs for rho and sigma
      ...
      hold<- Krig(x,y,Covariance="Matern",
              smoothness= smoothness, theta=theta.MLE,
               method="REML",m=m,Distance=Distance, ...)
...
# evaluate variogram
      if(is.null(Dmax)){
      Dmax<- (local.distance.function( cbind( range(x[,1]), range(x[,2]))))[2,1]}
      vg<- list()
      vg$x<- seq( 0,Dmax,,200)
      vg$y<- sigma.MLE^2  + rho.MLE*(1-Matern( vg$x/theta.MLE, smoothness=smoothness))
  return( list(smoothness=smoothness,theta.MLE=out[2],rho.MLE=out[1], sigma.MLE=out[3],
                     pars= out,REML=-REML, trA=trA, REML.grid= temp, vgram=vg) )
}
