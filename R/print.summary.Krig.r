...
function(x, ...)
{
	...
	print(sum, quote = F)
	cat("\n")
	cat("RESIDUAL SUMMARY:", fill = T)
	print(signif(res.quantile, digits))
	cat("\n")
	cat("COVARIANCE MODEL:", x$cov.function, fill = T)
	...
	if(x$knot.model) {
		cat(" Knot model: ", x$np, 
			" knots supplied to define basis functions", fill = T)
	}
	cat("\n")
	cat("DETAILS ON SMOOTHING PARAMETER:", fill = T)
	cat(" Method used:  ", x$method, "   Cost: ", x$cost, fill = T)
	#	cat(" Stats on this choice of lambda", fill = T)
	print(x$sum.gcv.lambda, digits = digits)
	cat("\n")
	cat(" Summary of estimates for lambda", fill = T)
