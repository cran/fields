mKrig.MLE<- function( x, y, ..., par.grid = NULL, lambda = NULL, lambda.profile = TRUE,  verbose = FALSE) 
{
# these are all the arguments needed to call mKrig except lambda and those in par.grid.
    mKrig.args <- c(list(x = x, y = y), list(...))
...
# output matrix to summarize results
    out <- matrix(NA, nrow = NG, ncol = 8)
    dimnames(out) <- list(NULL, c("EffDf", "lnProfLike", "GCV", "sigma.MLE",
                          "rho.MLE", "llambda.MLE", "counts eval","counts grad"))
    lambda.best<- NA
...
# NOTE: FULL refers to estimates collapsed across the replicates if Y is a matrix
        lnLike <- do.call( "mKrig",
                          c(mKrig.args,list( find.trA=FALSE), list( lambda = exp(x)),
                                            cov.args.temp) )$lnProfileLike.FULL
        return(lnLike) }
#
...
# final fit at optimal value (or starting value if not refinement/maximization for lambda)
        obj <- do.call("mKrig", 
                       c(mKrig.args, cov.args.temp, list(  lambda =  exp(llambda.opt))) )
...
# save results of the kth covariance model evaluation        
        out[k, 1:8] <- c(obj$eff.df, obj$lnProfileLike.FULL, obj$GCV, obj$sigma.MLE.FULL,
                         obj$rho.MLE.FULL, llambda.opt, optim.counts)
        if(verbose) {
            cat("Summary: ", k, out[k, 1:8], fill = TRUE) }
    }
    return( list(out = out, par.grid = par.grid, 
                 cov.args.MLE = cov.args.MLE, mKrig.args= mKrig.args,
                 lambda.best=lambda.best, call = match.call() ) )
}
