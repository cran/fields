mKrig.MLE<- function( x, y, weights = rep(1, nrow(x)), Z = NULL, ..., par.grid = NULL, lambda = NULL, lambda.profile = TRUE,  verbose = FALSE,
                     relative.tolerance= 1e-4) 
{
# these are all the arguments needed to call mKrig except lambda and those in par.grid.
    mKrig.args <- c(list(x = x, y = y, weights=weights, Z=Z), list(...))
...
# assign to hold only a few components returned by mKrig            
        hold <- do.call( "mKrig",
                   c(mKrig.args,list( find.trA=FALSE), list( lambda = exp(x)), cov.args.temp)
...
# add this evalution to an  object (i.e. here a matrix) in the calling frame        
         temp.eval<- get( "capture.evaluations") # current matrix of evaluations
...
# save results of the kth covariance model evaluation        
        summary[k, 1:8] <- c(obj$eff.df, obj$lnProfileLike.FULL, obj$GCV, obj$sigma.MLE.FULL,
    ...
    return( list(summary = summary, par.grid = par.grid, 
                 cov.args.MLE = cov.args.MLE, mKrig.args= list(...),
