...
"splint" <-
function (x, y, xgrid, wt=NULL, derivative = 0,lam=0, df=NA) 
{
#
# reform calling args if passed as a matrix or list
  if (is.matrix(x)) {
    ...
    if( any( duplicated(x))) {stop("duplicated x values, use sreg")}
...
# find lambda from eff degrees of freedom if it is passed
    if( !is.na(df)){
       if( (df<2)| (df>n)) { stop("df out of range")}
...
# call to old FORTRAN  
      .Fortran("css", h=as.double( ifelse(igcv==2,1,log(lam)) ),
        as.integer(n), as.double(x), 
        as.double(y), wt=as.double(1/sqrt(wt)),
