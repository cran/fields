QTps<- function(x,Y, ..., f.start=NULL, psi.scale=NULL, C=1, alpha=.5,
                Niterations=100, tolerance=1e-3,
                verbose=FALSE){
#  
#    
  good<- !is.na(Y)
...
# create Tps object to reuse for iterative fitting  
  ...
  for( k in 1:Niterations){
       Y.psuedo<- f.hat + C*psi.scale*qsreg.psi( (Y - f.hat)/psi.scale, C=C, alpha=alpha)
...
# One final complete fit at convergence.
   if( verbose){
      if( conv.flag){
          cat("Converged at tolerance", tolerance, "in", Number.iterations, "iterations", fill=TRUE) }
      else{
          cat("Exceeded maximum number of iterations", Niterations, fill=TRUE) }
    }
 # One final complete fit at convergence.
   f.hat<- f.hat.new
   Y.psuedo<- f.hat +  C*psi.scale*qsreg.psi( (Y - f.hat)/psi.scale, C=C, alpha=alpha)
...
#
        ...
        f.cv <- obj$fitted.values/(1 - diag.A) - diag.A * Y.psuedo/(1 - diag.A)
# leave-one-out estimate of f.hat
  CV.psuedo <- mean(qsreg.rho(Y - f.cv, alpha = alpha, C = psi.scale) )
# add extra stuff to the Krig object.    
   Qinfo<- list(yraw= Y, conv.info=conv.info, conv.flag=conv.flag, CV.psuedo=CV.psuedo,
                         psi.scale=psi.scale, alpha=alpha)    
   obj<- c( obj, list( Qinfo=Qinfo) )
   class( obj)<- "Krig" #can not find a better way to make this class Krig!
   return( obj) 
}
